...
export async function POST(req: Request) {
  try {
    ...
    const { userEmail, value, tier, useCustomCode, customCode } = body;

    if (!userEmail || !value || value <= 0 || !tier) {
      return NextResponse.json({ error: "User email, positive value, and tier required" }, { status: 400 });
    }
    ...
    if (useCustomCode && customCode) {
      ...
      if (!/^[A-Za-z0-9\-]+$/.test(customCode)) {
        return NextResponse.json({ error: "Custom code must be alphanumeric with hyphens only" }, { status: 400 });
      }
      // Check if custom code already exists
      const existing = await adminDb.collection("giftCodes").where("code", "==", customCode).limit(1).get();
      if (!existing.empty) {
    ...
    return NextResponse.json({
      message: "Gift code assigned and email sent successfully",
      code: {
        id: giftCodeId,
        code,
        value: parseFloat(value),
